...
export default function ExpiringProductCard({
  expiring,
  loading,
}: {
  expiring: any;
  ...
  return (
    <>
      ...
      >
        ...
        >
          <DefaultText fontWeight="bold">{t("Expiring Products")}</DefaultText>
        ...
        {expiring?.length > 0 ? (
          expiring?.map((data: any, index: number) => {
            return (
              <View key={index}>
                <View
                  style={{
                ...
                >
                  <View style={{ width: "50%" }}>
                    {loading ? (
                      ...
                      <DefaultText fontSize="lg" fontWeight="bold">
                        {isRTL
                          ? `${data.product.ar} ${data.variant.ar}`
                          : `${data.product.en} ${data.variant.en}`}
                      </DefaultText>
                    )}
                  ...
                  <View style={{ width: "45%", alignItems: "flex-end" }}>
                    {loading ? (
                      ...
                      <CurrencyView
                        amount={Number(
                          data.costPrice * data.totalAvailableQty || 0
                        ).toFixed(2)}
                        symbolFontsize={12}
                        amountFontsize={18}
                        decimalFontsize={12}
          ...
          >
            <NoDataPlaceholder
              title={t("no_data_expiring_products_list_in_inventory")}
            />
          </View>
        )}
