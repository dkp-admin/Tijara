...
export default function LostDamagedProductCard({
  lostOrDamaged,
  loading,
}: {
  lostOrDamaged: any;
    ...
    <>
      ...
      >
        ...
        {lostOrDamaged?.length > 0 ? (
          lostOrDamaged?.map((data: any, index: number) => {
            return (
              <View key={index}>
                <View
                  style={{
                ...
                >
                  <View style={{ width: "50%" }}>
                    {loading ? (
                      ...
                      <DefaultText fontSize="lg" fontWeight="bold">
                        {isRTL
                          ? `${data.productName.ar} ${data.variantName.ar}`
                          : `${data.productName.en} ${data.variantName.en}`}
                      </DefaultText>
                    )}
                  ...
                  <View style={{ width: "45%", alignItems: "flex-end" }}>
                    {loading ? (
                      ...
                      <CurrencyView
                        amount={Number(data.price * data.totalQty || 0).toFixed(
                          2
                        )}
                        symbolFontsize={12}
                        amountFontsize={18}
          ...
          >
            <NoDataPlaceholder
              title={t("no_data_low_damaged_products_list_in_inventory")}
            />
          </View>
        )}

        {lostOrDamaged?.length > 0 && !loading && (
          <TouchableOpacity
            style={{
