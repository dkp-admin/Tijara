...
const BarcodePrinting = () => {
  ...
  const handleBarcodePrint = async () => {
    const allprinter = await repository.printerRepository.findByType("usb");
    const printer = allprinter.find((p) => p.enableBarcodes);
    ...
    for (const product of barcodeData?.products) {
      ...
      const data = {
        ...
        price: Number(product.variant?.prices?.[0]?.price || 0),
  ...
  return (
    <>
      ...
      <ActionSheetHeader
        isClose={false}
        handleLeftBtn={() =>
          navigation.navigate("MainNavigator", { screen: "MoreHome" })
        }
