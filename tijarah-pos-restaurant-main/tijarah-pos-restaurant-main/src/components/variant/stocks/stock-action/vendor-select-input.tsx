...
export default function VendorSelectInput({
  sheetRef,
  values,
  handleSelected,
}: {
  ...
  const getVendors = async () => {
    ...
    try {
      const res = await serviceCaller(endpoint.vendor.path, {
        method: endpoint.vendor.method,
        query: {
          page: 0,
          limit: 25,
          _q: query,
        ...
        {isConnected ? (
          <View>
            <Input
              ...
              placeholderText={t("Search vendor with name")}
              values={query}
              allowClear
                ...
                return (
                  <>
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        handleSelected({ value: item.name, key: item._id });
                      }}
                    >
                      <DefaultText
                        fontWeight={
                ...
                >
                  {vendor.length === 25 && (
                    <DefaultText fontWeight="medium" color="otherGrey.200">
                      {t("Type in the search bar to find more vendors")}
                    </DefaultText>
                  )}
                </View>
              )}
