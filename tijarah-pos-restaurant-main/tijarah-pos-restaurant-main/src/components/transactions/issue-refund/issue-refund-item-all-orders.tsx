...
import ItemRowOrders from "./item-row-orders";
import RestockItemsModal from "./restock-items";

export default function IssueRefundItemModalOrders({
  data,
  visible = false,
  handleClose,
  handleIssueRefund,
        ...
        if (count != 0) {
          for (let index = 0; index < count; index++) {
            ...
            itemsData.push({
              ...
              noOfUnits: item.variant?.noOfUnits,
              amount: Number(
                item.billing?.total /
                  (item.variant?.unit === "perItem" ||
                  item.variant?.type === "box" ||
              ...
              costPrice: item.billing?.costPrice,
              vat: Number(
              ...
              hasMultipleVariants: item.variant?.hasMultipleVariants,
              availability: item.variant?.stock?.availability,
              stockCount: item.variant?.stock?.stockCount,
