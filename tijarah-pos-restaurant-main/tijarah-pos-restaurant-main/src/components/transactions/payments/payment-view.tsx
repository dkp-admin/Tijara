...
export default function PaymentView({
  ...
  const paymentData = (data: any, index: number) => {
    ...
    } else if (data.providerName === PROVIDER_NAME.WALLET) {
      return walletData(data);
    } else return otherPaymentTypes(data);
  };
  ...
  const cashData = (data: any) => {
    ...
    if (Number(data?.change || 0) > 0) {
      payment.push({
        title: t("Tendered"),
        value: `${currency} ${Number(data?.total || 0)?.toFixed(2)}`,
      });

      payment.push({
        title: t("Change"),
        value: `${currency} ${Number(data?.change || 0)?.toFixed(2)}`,
      });
    }

    payment.push({
      title: t("Cash"),
      value: `${currency} ${Number(cash || 0)?.toFixed(2)}`,
    });
  ...
  const otherPaymentTypes = (data: any) => {
    const obj: any = {
        ...
        >
          ...
          >
            {`${cashier} (${device}), ${format(
              new Date(createdAt || data?.createdAt),
              "dd/MM/yyyy, hh:mma"
            )}`}
          </DefaultText>
        </View>
