...
import SyncProgressModal from "./components/sync-in-progress-modal";
...
const MoreHome = () => {
  ...
  const checkHeartbeat = async (): Promise<boolean> => {
    try {
      ...
      await fetch(`${HOST}/authentication/heartbeat`, requestOptions);
  ...
  const handleSyncRefresh = async () => {
    const isHeartbeatSuccessful = await checkHeartbeat();

    if (!isHeartbeatSuccessful) {
      console.log("Heartbeat check failed");
      showToast("error", t("500_message"));
      return;
    }
    ...
    await databasePull.fetchAllEntities();
  };
