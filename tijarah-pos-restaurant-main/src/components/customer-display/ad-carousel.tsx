...
const AdCarousel = ({ data }: any) => {
  ...
  useEffect(() => {
    ...
    console.log(
      `Setting up interval for slide ${activeIndex} with duration ${data[activeIndex]?.duration} seconds`
    );

    const interval = setInterval(() => {
      setActiveIndex((prevIndex: any) => {
        const nextIndex = prevIndex < data.length - 1 ? prevIndex + 1 : 0;
        console.log(`Slide completed: ${prevIndex}, moving to ${nextIndex}`);
        ref.current?.next();
        EventRegister.emit("slideCompeleted", prevIndex);
        return nextIndex;
      });
    }, (data[activeIndex]?.duration || 60) * 1000); // Default to 60 seconds if duration is not set

    return () => {
      console.log(`Clearing interval for slide ${activeIndex}`);
