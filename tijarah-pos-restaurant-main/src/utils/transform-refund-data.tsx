...
const refundBreakups = (order: any) => {
  const methods = ["cash", "card", "wallet", "credit", "stcpay"];
  ...
  const paidWithOther = order.refunds?.[0]?.refundedTo
    ?.filter((p: any) => !methods.includes(p.providerName))
    ?.reduce((pv: any, cv: any) => pv + cv.total, 0);

  if (Number(paidWithOther) > 0) {
    const paidWithOtherName = order.refunds?.[0]?.refundedTo?.filter(
      (p: any) => !methods.includes(p.providerName)
    );
    refundTo.push({
      text: Device.brand === "qcom" ? "card" : paidWithOtherName[0].name,
      value: Number(paidWithOther || 0).toFixed(2),
    });
  }
