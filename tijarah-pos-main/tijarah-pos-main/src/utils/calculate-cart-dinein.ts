import dineinCart from "./dinein-cart";
import { getItemSellingPrice, getItemVAT } from "./get-price";

const calculateCartDinein = () => {
  const useCartStore = require("../store/cart-item-dinein").default;
  const items = dineinCart.getCartItems();
  const discountsApplied = useCartStore.getState().discountsApplied;
  ...
  const totalPaidAmount = order?.payment?.breakup?.reduce(
...
export default calculateCartDinein;
