  ...
  const sendOTPForCredit = async () => {
    ...
    try {
      ...
      if (res.code === "otp_sent") {
        debugLog(
          "OTP sent for using credit limit",
          res,
          "credit-transaction-modal",
          "sendCreditOTPFunction"
        );
    ...
    (async () => {
      if (customer?._id) {
        ...
        try {
          ...
          if (res) {
            debugLog(
              "Customer credit fetched from api",
              res,
              "credit-transaction-modal",
              "fetchCustomerCredit"
            );
              ...
              {customer?._id ? (
                <View>
                  ...
                  <PrimaryButton
                    ...
                    title={`${t("SAR")} ${Number(creditAmount).toFixed(
                      2
                    )} \n ${t("from Credit")}`}
                    ...
                    onPress={() => {
                      if (!customerData?.allowCredit) {
                        ...
                        const message = `${t("Used amount upto")} ${t(
                          "SAR"
                        )} ${getTotalAmount()}`;
                        debugLog(
                          message,
                          {
                            customerCredit: customerData?.availableCredit,
                          },
                          "credit-transaction-modal",
                          "handleOnPressCreditFunction"
                        );
      ...
      <WalletCustomerModal
        ...
        handleSelectedCustomer={(customer: any) => {
          debugLog(
            ...
            customer,
            "credit-transaction-modal",
            "handleAddCustomerCreditunction"
          );
