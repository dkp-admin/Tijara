...
export default function WalletRedeemOTPModal({
  data,
  visible = false,
  handleClose,
  handleRedeem,
  ...
  const handleRedeemWallet = async () => {
    if (otp.length == 0) {
      debugLog(
        "OTP not entered",
        { otp: otp },
        "redeem-wallet-otp-modal",
        "handleRedeemWalletFunction"
      );
    ...
    try {
      ...
      if (res.code == "success") {
        debugLog(
          "OTP verified for redeem wallet amount",
          res,
          "redeem-wallet-otp-modal",
          "handleRedeemWalletFunction"
        );
        ...
        MMKVDB.set(DBKeys.WALLET_OTP_WRONG_ATTEMPTS, "0");
        MMKVDB.set(DBKeys.WALLET_OTP_BLOCKED_UNTIL, "");
  ...
  const handleWrongAttempt = () => {
    ...
    MMKVDB.set(DBKeys.WALLET_OTP_WRONG_ATTEMPTS, `${wrongAttempts + 1}`);
    ...
    MMKVDB.set(DBKeys.WALLET_OTP_BLOCKED_UNTIL, blockedTime);
  };

  const fetchData = async () => {
    // Retrieve stored wrong attempts count and blockedUntil timestamp
    const storedWrongAttempts =
      MMKVDB.get(DBKeys.WALLET_OTP_WRONG_ATTEMPTS) || "0";

    const storedBlockedUntil = MMKVDB.get(DBKeys.WALLET_OTP_BLOCKED_UNTIL);
      ...
      >
        ...
        >
          ...
          <DefaultText
            ...
            fontWeight="medium"
          >
            {`${t("Redeeming")} ${t("SAR")} ${data?.walletAmount}`}
          </DefaultText>
