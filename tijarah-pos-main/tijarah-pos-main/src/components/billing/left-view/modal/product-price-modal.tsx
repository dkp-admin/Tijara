...
export default function ProductPriceModal({
  ...
  const getVariantPrice = useMemo(
    () => (item: any) => {
      if (selectedVariant?.sku == item?.sku && selectedVariant?.isOpenPrice) {
        ...
        return `${t("SAR")} ${Number(item.prices[0].price)?.toFixed(2)}`;
      } else {
        return t("Custom");
      }
    },
    ...
    const handlePress = () => {
      if (prodData.notBillingProduct) {
        debugLog(
          ...
          item,
          "billing-screen",
          "handleVariantProductPriceFunction"
        );
        showToast("error", t("Looks like the item is out of stock"));
        return;
      }

      if (Number(item.prices[0].price) > 0) {
  ...
  const handleChange = useCallback(() => {
    ...
    debugLog(
      "Variant selected for cart",
      selectedVariant,
      "billing-screen",
      "handleChangeProductPriceFunction"
    );
  ...
  useEffect(() => {
    if (visible) {
      ...
      if (data?.variants?.length == 1 && variant.prices[0]?.price) {
        setSelectedVariant({
          _id: variant._id,
    ...
    >
              ...
              {data?.variants?.length < 1 && (
                <>
                  ...
                  <Input
                    ...
                    values={`${t("SAR")} ${selectedVariant?.sellingPrice}`}
                    handleChange={(val: any) => {}}
                    disabled
                  />
                </>
      ...
      <ProductCustomPriceModal
        ...
        handlePriceModalClose={(item: any) => {
          ...
          setTotalPrice((item?.sellingPrice || 0)?.toFixed(2));
