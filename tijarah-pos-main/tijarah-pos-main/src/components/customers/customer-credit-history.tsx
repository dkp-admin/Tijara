...
export default function CustomerCreditHistory({
  creditHistory,
}: {
  creditHistory: any[];
}) {
  ...
  const getAmount = (credit: any) => {
    if (credit.transactionType === "credit") {
      return `+${Number(credit.amount || 0)?.toFixed(2)}`;
    } else {
      return `-${Number(credit.amount || 0)?.toFixed(2)}`;
    }
  };
  ...
  return (
    <View>
      ...
      <Label>
        {`${t("Credit History")} - (${t("Showing latest 5 history")})`}
      </Label>

      {creditHistory.map((wallet: any, index: number) => {
        const length = creditHistory.length - 1;
