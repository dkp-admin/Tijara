...
export default function AddEditCustomerModal({
  ...
  const formik: FormikProps<AddCustomerProps> = useFormik<AddCustomerProps>({
    ...
    onSubmit: async (values) => {
      ...
      if (
        ...
        showToast(
          "error",
          `${t("Max credit should be less than")} ${t("SAR")} ${credit?.toFixed(
            2
          )}`
        );
        return;
      ...
      try {
        ...
        if (res !== null) {
          ...
          debugLog(
            data?.id
              ? "Customer updated to server"
              : "Customer created to server",
            res,
  ...
  const handleDateChange = (
    ...
    const dateList = formik.values.importantDates.map((data: any) => {
      if (data.name == name) {
        debugLog(
          "Change special event date for customer",
          { _id: _id, name: name, type: type, date: val },
          "customer-create-modal",
          "handleDateChange"
        );
  ...
  const getCustomerWalletHistory = async () => {
    ...
    try {
      ...
      debugLog(
        "Customer wallet history fetch from api",
        {},
        "customer-create-modal",
        "customerWalletHistory"
      );
  ...
  const getCustomerCreditHistory = async () => {
    ...
    try {
      ...
      debugLog(
        "Customer credit history fetch from api",
        {},
        "customer-create-modal",
                            ...
                            infoMsg={
                              ...
                              "LIMIT_CREDIT"
                                ? `${t(
                                  ...
                                  )} ${t("Max credit allowed")}: ${t(
                                    "SAR"
                                  )} ${getMaxCreditLimit().toFixed(2)}`
                                : t(
                                    "info_msg_max_credit_message_for_customer_screen"
                                  )
                            }
