...
export default function TaxSelectInput({
  sheetRef,
  values,
  handleSelected,
}: {
  ...
  const getTaxess = async () => {
    if (!isConnected) {
      setTaxes([]);
    }

    try {
      const res = await serviceCaller(endpoint.tax.path, {
        method: endpoint.tax.method,
        query: {
          _q: "",
          page: 0,
          limit: 25,
      ...
      debugLog(
        "Taxes fetch from api",
        res?.results?.length,
        "tax-select-input",
        "fetchTaxes"
      );

      setTaxes(res?.results || []);
    } catch (error: any) {
      errorLog(error?.message, {}, "tax-select-input", "fetchTaxes", error);
      setTaxes([]);
    }
  };
              ...
              return (
                <>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      handleSelected({ value: item.tax, key: item._id });
                    }}
                  >
                    <DefaultText
