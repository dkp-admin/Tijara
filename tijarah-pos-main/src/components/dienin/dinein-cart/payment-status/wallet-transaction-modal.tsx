...
import WalletRedeemOTPModal from "../../../billing/right-view/modal/wallet-otp-modal";
...
export default function DineinWalletTransactionModal({
  visible = false,
  handleClose,
  onChange,
  totalAmount = 0,
  ...
  const sendOTPForWallet = async () => {
    ...
    try {
      ...
      if (res.code === "otp_sent") {
        debugLog(
          "OTP sent for redeem wallet amount",
          res,
          "wallet-transaction-modal",
          "sendWalletOTPFunction"
        );
  ...
  useEffect(() => {
    (async () => {
      if (customer?._id && businessDetails?._id) {
        if (!isConnected) {
          infoLog(
            "Internet not connected",
            customer,
        ...
        try {
          ...
          if (res) {
            debugLog(
              "Customer wallet fetched from api",
              res,
              "wallet-transaction-modal",
              "fetchCustomerWallet"
            );
                  ...
                  <PrimaryButton
                    ...
                    onPress={() => {
                      if (!selected?.text) {
                        ...
                        debugLog(
                          message,
                          {
                            minimumWalletBalance:
                              businessDetails?.company?.minimumWalletBalance,
                          },
                          "wallet-transaction-modal",
                          "handleOnPressdWalletFunction"
                        );
                        ...
                        const message = `${t("Redeem amount upto")} ${t(
                          "SAR"
                        )} ${getTotalAmount()}`;
                        debugLog(
                          message,
      ...
      <WalletCustomerModal
        ...
        handleSelectedCustomer={(customer: any) => {
          debugLog(
            ...
            customer,
            "wallet-transaction-modal",
            "handleAddCustomerWalletFunction"
          );
