...
export async function printSunmi2Inch(order: any) {
  debugLog(
    "Sunmi 2 inch order print started",
    order,
    "order-print-sunmi-2-Inch",
    "printSunmi2InchFunction"
  );
  ...
  order.items.forEach((it: any) => {
    ...
    }${it.variant.sellingPrice?.toFixed(2)}               ${it.quantity}      ${
      it.billing.total
    }\n`;
  });
  ...
  order.payment?.charges?.forEach((charge: any) => {
    receiptContent +=
      constructString(charge.name.en, `SAR ${charge.total.toString()}`) + "\n";
  ...
  debugLog(
    "Sunmi 2 inch order print completed",
    order,
    "order-print-sunmi-2-Inch",
    "printSunmi2InchFunction"
  );
